...
var ABNF_ABNF string = `rulelist = 1*(rule / (*WSP c-nl))
...
func Test_ABNF(t *testing.T) {
	ret := strings.Split(ABNF_ABNF,"\n")
	sort.Strings(ret)
...
func Test_ABNF_ABNF(t *testing.T) {
	ret := strings.Split(ABNF_ABNF,"\n")
	sort.Strings(ret)
	sorted_ABNF_ABNF := []byte(strings.Join(ret,"\r\n"))

	tested_ABNF := ABNF().Valid(sorted_ABNF_ABNF)
	if !tested_ABNF {
